	...
	while ((tok = lexer_next(lex)).type != TOKEN_EOF) {
		switch (tok.type) {
			...
			case TOKEN_CTIMEDEF_START:
				...
				while ((tok = lexer_next(lex)).type != TOKEN_CTIMEDEF_END) {
					if (tok.type == TOKEN_STRING) {
						...
						return_expr_fn = ctt_format("const char *__comptime_insert_layer%d_%d() { return %s; }\n", parent_scope, insert_counters[parent_scope]++, tok.string_data);
						layer_tails[scope] = block_list_append(layer_tails[scope], make_block(scope, B_SOURCE_CODE, return_expr_fn));
			...
			case TOKEN_INSERTION_START:
				...
				return_expr_fn = ctt_format("const char *__comptime_insert_target_%d() { return %s; }\n", source_insert_counter++, tok.string_data);
				layer_tails[scope] = block_list_append(layer_tails[scope], make_block(scope, B_SOURCE_CODE, return_expr_fn));
				source_tail = block_list_append(source_tail, make_block(scope, B_SOURCE_INSERT, return_expr_fn));
				tok = match(lex, TOKEN_INSERTION_END);
				if (tok.scope != scope) {
